/**
 * TESTING PLAN — ExerciseFilterBar
 *
 * COVERED:
 *  - All 8 filter buttons render (ALL, CHEST, BACK, LEGS, SHOULDERS, BICEPS, TRICEPS, ABS)
 *  - Active filter button has the 'active' CSS class
 *  - Inactive buttons do NOT have the 'active' class
 *  - onFilterChange called with correct label on button click
 *  - onFilterChange called exactly once per click
 *  - Clicking all 8 buttons fires onFilterChange 8 times
 *  - Clicking ALL calls onFilterChange("ALL")
 *
 * TODO — Gaps to fill:
...
describe("ExerciseFilterBar", () => {
  ...
  it("calls onFilterChange with the correct filter label when clicked", () => {
    ...
    render(
      <ExerciseFilterBar activeFilter="ALL" onFilterChange={onFilterChange} />,
    );
